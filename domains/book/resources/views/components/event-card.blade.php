...
<div class="flex gap-2 sm:gap-5">
    ...
    <div class="">
        ...
        <p class="text-xl font-bold text-amber-950">
            {{ $event->scheduled_at->format('H:i') . ' - ' . $event->finished_at->format('H:i') }}</p>
    </div>
</div>
<div class="pl-4 mt-2 space-y-2 w-fit">
    <p><x-heroicon-s-map-pin class="inline w-6 h-6 mb-1 text-amber-800/50" /> {{ $event->location->name }},
        {{ $event->location->address }}
        <span style="white-space: nowrap;">(<a href="{{ $event->location->google_maps_url }}" class="inline-link"
                rel=“noreferrer”>Maps<x-heroicon-o-arrow-top-right-on-square
                    class="inline w-4 h-4 mb-1 ml-1" /></a>)</span>
    </p>

    @if ($event->going >= 10)
        <p class="font-semibold"><x-heroicon-s-users class="inline w-6 h-6 mb-1 text-amber-800/50" />
            {{ $event->going }} Going</p>
    @endif
