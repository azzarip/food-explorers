...
@section('second_column')
    ...
    <div class="max-w-md mx-auto">
        ...
        <form action="" id="payment-form" class="space-y-4">
            <div>
                <div id="payment-element" data-stripe-key="{{ config('services.stripe.key') }}"
                    data-client-secret="{{ $clientSecret }}" data-return-url="{{ $return_url }}">
                </div>
                <p class="mt-1 text-xs text-center text-slate-400"><x-heroicon-s-lock-closed class="inline w-3 h-3 mb-1" />
                    SSL-encrypted connection </p>
            </div>
            <x-forms.agb />
